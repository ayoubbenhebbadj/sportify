...
<div class="container">
  <div class="top-bar">
    ...
    <button class="btn-ajouter" onclick="ouvrirFormulaire()">Ajouter un utilisateur</button>
  </div>
  <!-- Add User Modal -->
  <div id="addUserModal" class="modal">
    <div class="modal-content">
      ...
      <form id="addUserForm">
        ...
        <div class="input-group">
          <select class="input" name="clubtype" required>
            <option value="">Type de club</option>
      ...
      <?php
        ...
        if ($res && mysqli_num_rows($res) > 0) {
          while ($row = mysqli_fetch_assoc($res)) {
            ...
            $nom = $row['lastname'];
            $statut = $row['state']; // 'active' or 'suspended'
...
<script>
  ...
  document.getElementById('addUserForm').onsubmit = function(e) {
    ...
    .then(res => res.text())
    .then(response => {
      // If PHP redirects, response will be empty, so just reload
